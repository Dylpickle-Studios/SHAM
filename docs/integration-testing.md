...
the `v1.1.2` public stable predecessor (or an explicit
...
the full tag history and requires a stable baseline. The baseline checkout is
run unmodified: its application logic, database schema, persisted data, and
migration paths are the tagged release's own. The restore drill
